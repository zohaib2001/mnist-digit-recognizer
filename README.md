...
* **Accurate Blank Detection:** The app only runs inference when an actual stroke is drawn — no phantom predictions on an empty canvas.
* **Clean Reset:** The 🗑️ Clear button fully remounts the canvas so there are no ghost artifacts from the previous drawing.
* **Adaptive Theming:** The UI works in both Streamlit's light and dark modes with correct contrast throughout.
...
```
Your drawing  →  Resize to 28×28px  →  Normalize pixels to 0–1  →  Neural network  →  Prediction + confidence
```
...
**4. Train the model** — open `train.ipynb` in VS Code (or preferred IDE) and run all cells in order. This downloads MNIST automatically and saves `model.pth` when complete.

**Expected training output:**
```
Epoch 1/5  loss: 0.3821
Epoch 2/5  loss: 0.1634
...
>`model.pth` must exist before running the app. Complete step 4 first.
...
├── model.pth          # Saved model weights (generated by train.ipynb)
│
